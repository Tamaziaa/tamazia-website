...
window.CH = (function(){
  ...
  function engineGrid(){
    return `<div class="enggrid">${D.geo.engines.map(e=>{
      ...
      return `<div class="engcell ${e.cites?'':'no'}"><div class="eng-id"><span class="eng-logo" style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;${e.cites?'':'opacity:.42;filter:grayscale(1)'}">${ENG_LOGO[slug]||''}</span><span class="eng-nm" style="font-family:var(--mono);font-size:10px;color:var(--ink)">${esc(e.nm)}</span></div>
