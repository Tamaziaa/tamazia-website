...
### S10 [OS-V4 #38] · NEW Website Remodelling page · `src/pages/services/website-remodelling.astro` (+ link from `src/components/sections/Pricing.astro`)
- **NEW page** (was 404). Built on BaseLayout + the real Header/Footer + FleuronDivider, styled entirely with the canonical tokens.css design tokens (all 13 vars verified to exist). Price comes from the ONE config: `independentSolutionsGbp.websiteRemodelling` → struck anchor £4,800 then offer £2,400 (one time), and the audit reference reads `entryAuditGbp` (£1,500). Sections: hero + price plate, "What the engagement covers" (6 inclusions), "Who it is for", and an audit-led close. Appends a Service JSON-LD node via BaseLayout `extraSchema` (provider `@id` = `#organization`, with an Offer at £2,400). Indexable, so the sitemap picks it up at the `/services/` priority 0.85.
- **Copy:** strict third-person, NO we/our/us anywhere in visible copy (verified), NO em dashes / hyphen-pauses (verified). Prices never hardcoded.
- **Linked from:** the Pricing Independent Solutions "Website Remodelling" card — added an optional `href` on the solution meta (only this solution has a standalone page); its CTA now reads "See the engagement" and deep-links to `/services/website-remodelling/`, while the other 7 solutions keep the `#contact` "Add this" CTA.
- jsc (extracted frontmatter): PASS. Astro structural review: fences balanced; `<>`/`</>` 1:1; `<section>` 4:4; all imports + CSS vars resolve.
