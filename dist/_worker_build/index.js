...
async function fireAlert2(env, tab, body, request_id) {
  const html = `<h2 style="font-family:Georgia,serif">New ${tab} submission</h2>
    <p>Request <code>${request_id}</code></p>
    <table style="border-collapse:collapse;font-family:system-ui;font-size:14px">${Object.entries(body).map(([k, v]) => `<tr><td style="padding:6px 12px;background:#f5f5f5;font-weight:600">${esc2(k)}</td><td style="padding:6px 12px">${esc2(String(v).slice(0, 500))}</td></tr>`).join("")}</table>`;
  return fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { "Authorization": "Bearer " + env.RESEND_API_KEY, "Content-Type": "application/json" },
    body: JSON.stringify({
...
__name(json2, "json");
var onRequestPost3 = /* @__PURE__ */ __name(({ request, env }) => handleSubmission2(request, env, "contact"), "onRequestPost");
