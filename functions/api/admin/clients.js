...
export const onRequestGet = async ({ request, env }) => {
  ...
  try {
    const r = await fetch('https://' + host + '/sql', { method: 'POST', headers: { 'Neon-Connection-String': env.NEON_URL, 'Content-Type': 'application/json' }, body: JSON.stringify({ query: "SELECT ca.id, ca.legal_name, ca.tier, ca.deal_value, ca.currency, ca.status, ca.health_score, ca.contract_start, ca.contract_end, l.company, l.domain FROM client_accounts ca LEFT JOIN leads l ON l.id=ca.lead_id ORDER BY ca.created_at DESC LIMIT 200", params: [] }) });
